...
valid_years = [year for year in range(2016,3000,1)]
...
class VereinManager(tk.Frame):
    ...
    def login_page(self):
        ...
        self.first_frame = tk.Frame(self.master, bg=background_color)
        self.first_frame.grid(row=5, column=0, padx=250, pady=150)
    ...
    def check_database(self):
        ...
        self.cur.execute("""
                        CREATE table IF NOT EXISTS member_data(Id INT PRIMARY KEY, ref_no TEXT, name TEXT, street TEXT, house_no TEXT, post_code TEXT, city BLOB, phone TEXT,
        ...
        self.cur.execute("""
                        CREATE table IF NOT EXISTS deleted_member_data(Id, ref_no TEXT, name TEXT, street TEXT, house_no TEXT, post_code TEXT, city BLOB, phone TEXT,
        ...
        self.cur.execute("""
                        CREATE table IF NOT EXISTS friday_data(Id INT , date TEXT, amount REAL)
            """)
    ...
    def start_page(self,check=False):
        ...
        self.first_frame.destroy()
        self.first_frame = tk.Frame(self.master, bg=background_color)
        self.first_frame.grid(row=5, column=0, padx=250, pady=75)
    ...
    def members_page(self):
        root.title("Verein Manager/Members")
        global all_member_data
        all_member_data=[]
        self.first_frame.destroy()
        self.first_frame = tk.Frame(self.master, bg=background_color)
    ...
    def import_export_data(self):
        root.title("Verein Manager/Import-Export Data")
        self.first_frame.destroy()
        self.first_frame = tk.Frame(self.master,bg=background_color)
        self.first_frame.grid(row=0)
        but_frame = tk.Frame(self.first_frame,bg=background_color)
        but_frame.grid(row=1, column=1,padx=250,pady=100)
    ...
    def add_member_data(self):
        root.title("Verein Manager/Members/Add New")
        self.checkbox_pane.destroy()
        self.first_frame.destroy()
        self.first_frame = tk.Frame(self.master, bg=background_color)
    ...
    def add_record(self,from_import=""):
        ...
        if self.all_fields_verified and self.no_empty_field:
            ...
            parameters = [id, self.ref_no.get().zfill(4), string.capwords(self.f_name.get()), string.capwords(self.street.get()), self.house_no.get(),  
                          self.post_code.get(), string.capwords(self.city.get()), self.phone.get(), self.date_of_birth.get(), donation, self.email.get(),
    ...
    def edit_record(self):
        root.title("Verein Manager/Members/Edit")
        global all_member_data
        if not all_member_data:
    ...
    def manage_finance(self, kill_checkbox=""):
        root.title("Verein Manager/Finance")
        global all_member_data
        all_member_data=[]
        self.first_frame.destroy()
        if kill_checkbox:
        ...
        b1 = tk.Button(but_frame, text="Update", command=lambda: self.select_month_year(method_to_call="finance"), height=1, width=15)
        b1.grid(row=1, column=1, padx=25, pady=25)
        b4 = tk.Button(but_frame, text="Check Payments", command=lambda: self.select_month_year(method_to_call="check_payments"), height=1, width=15)
        b4.grid(row=1, column=2, padx=5, pady=10)
        b5 = tk.Button(but_frame, text="Friday Data", command=self.friday_data, height=1, width=15)
        b5.grid(row=1, column=3, padx=25, pady=10)
        b7 = tk.Button(but_frame, text="View Monthly Data", command=self.view_finance_data_by_month_year, height=1, width=15)
        b7.grid(row=2, column=1, padx=5, pady=10)
        b2 = tk.Button(but_frame, text="View Yearly Data", command=self.view_yearly_financial_data, height=1, width=15)
        b2.grid(row=2, column=2, padx=5, pady=10)
    ...
    def comm_with_member(self,check_des=""):
        root.title("Verein Manager/History/Communication")
        if check_des:
            self.checkbox_pane.destroy()
        self.first_frame.destroy()  
        self.first_frame = tk.Frame(self.master, bg=background_color)
        ...
        field_fr.grid(row=0,column=0, padx=250,pady=100,sticky=tk.S)
        but_fr = tk.Frame(self.first_frame,bg=background_color)
    ...
    def check_current_payments(self,year,kill_checkbox=""):
        if int(year) not in valid_years:
            tkMessageBox.showerror("View Monthly Data", "Enter a valid year to view data")
        else:
            self.selected_year.set(year)
            root.title("Verein Manager/Finance/Check Payments")
            if kill_checkbox:
                self.checkbox_pane.destroy()
            self.first_frame.destroy()
            self.first_frame = tk.Frame(self.master, bg=background_color)
    ...
    def get_payment_data(self,defaulter=False,save_msg=False):
        ...
        all_comm_recs=[]
        if defaulter:
            root.title("Verein Manager/Finance/Check Payments/Defaulter")
            ...
            root.title("Verein Manager/Finance/Check Payments/Non-Defaulter")
            sql = "Select * from finance inner join member_data on finance.Id = member_data.Id where finance.actual_amt >= finance.donated_amt and finance.month=? and finance.year =?"
            parameters = [self.selected_month.get(), self.selected_year.get()]
            self.cur.execute(sql, parameters)
            all_data = self.cur.fetchall()
        ...
        but_fr_1 = tk.Frame(self.first_frame, bg=background_color)
        but_fr_1.grid(row=1, column=1, padx=10,pady=1)
        self.next_but = tk.Button(but_fr_1, text = 'Next', height = 1, width = 12)
        self.prev_but = tk.Button(but_fr_1, text = 'Previous', height = 1, width = 12)
        ...
        but_fr = tk.Frame(self.first_frame, bg=background_color)
        but_fr.grid(row=1, column=3, padx=100,pady=1)
        ...
        prepare_but.grid(row=3, column=1, padx=5, pady=5)
        edit_but = tk.Button(but_fr, text = 'Edit Record', command =lambda: self.edit_payment_record(all_comm_recs,defaulter), height = 1, width = 12)
        edit_but.grid(row=3, column=2, padx=5, pady=5)
        back_but = tk.Button(but_fr, text = 'Back', command = lambda: self.check_current_payments(year=self.selected_year.get(), kill_checkbox=True), height = 1, width = 12)
        back_but.grid(row=4, column=1, padx=5, pady=5)
        lab_fr = tk.Frame(self.first_frame, bg=background_color)
        ...
        if not all_data:
            ...
            edit_but.config(state=tk.DISABLED)
            self.next_but.config(state=tk.DISABLED)
            self.prev_but.config(state=tk.DISABLED)
            ...
            edit_but.config(state=tk.NORMAL)
            self.max_len = len(all_data)
            if self.max_len <= self.max_num:
                self.max_num = self.max_len
                self.next_but.config(state=tk.DISABLED)
    ...
    def edit_payment_record(self, record, defaulter):
        if len(record) < 1:
            tkMessageBox.showwarning("Edit Payment", "Select at least 1 record to edit!")
        elif len(record) > 1:
            tkMessageBox.showwarning("Edit Payment", "Select only 1 record to edit!")
        else:
            self.checkbox_pane.destroy()
            self.first_frame.destroy()
            self.first_frame = tk.Frame(self.master, bg=background_color)
            ...
            frame.grid(row=5, column=0, pady=150, padx=200)
            ref_no,name,exp_amt,act_amt = record[0].split(";")
            ref_no_label = tk.Label(frame, text="Ref. No :", width=10, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            name_label = tk.Label(frame, text="Name :", width=10, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            exp_amt_label = tk.Label(frame, text="Expected Amount :" , width=15, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            act_amt_val_label = tk.Label(frame, text="Actual Amount :", width=15, justify=tk.CENTER,anchor=tk.W, bg=background_color)
            ref_no_val_label = tk.Label(frame, text=ref_no.zfill(4), width=10, justify=tk.CENTER,anchor=tk.W, bg=background_color)
            name_val_label = tk.Label(frame, text=name, width=25, justify=tk.CENTER,anchor=tk.W, bg=background_color)
            exp_amt_val_label = tk.Label(frame, text=exp_amt, width=15, justify=tk.CENTER,anchor=tk.W, bg=background_color)
            actual_amt = tk.Entry(frame, width=5)
            ref_no_label.grid(row=0, column=0, sticky=tk.W, padx=15, pady=5)
            name_label.grid(row=1, column=0, sticky=tk.W, padx=15, pady=5)
            exp_amt_label.grid(row=2, column=0, sticky=tk.W, padx=15, pady=5)
            act_amt_val_label.grid(row=3, column=0, sticky=tk.W, padx=15, pady=5)
            ref_no_val_label.grid(row=0, column=1, sticky=tk.W)
            name_val_label.grid(row=1, column=1, sticky=tk.W)
            exp_amt_val_label.grid(row=2, column=1, sticky=tk.W)
            actual_amt.grid(row=3,column=1, sticky=tk.W)
            actual_amt.insert(0, act_amt)
            edit_but = tk.Button(frame, text = 'Save Record', command =lambda: self.update_payment_record(ref_no,actual_amt.get(),exp_amt,name,defaulter), height = 1, width = 12)
            edit_but.grid(row=5, column=0, padx=5, pady=35)
            back_but = tk.Button(frame, text = 'Back', command = lambda: self.get_payment_data(defaulter=defaulter,save_msg=True), height = 1, width = 12)
            back_but.grid(row=5, column=1, padx=5, pady=5)
        
    def update_payment_record(self, ref_no, amount, expected_amt, name, defaulter):
        actual_month=self.selected_month.get()
        actual_year=self.selected_year.get()
        try:
            amount = float(amount)
        except Exception, msg:
            tkMessageBox.showwarning("Update Amount", "Enter valid amount to update")
        else:
            sql = "Select * from finance where ref_no=? and month=? and year=?"
            self.cur.execute(sql, [ref_no.zfill(4),string.capwords(actual_month),actual_year])
            result = self.cur.fetchall()
            if result:
                sql = "Update finance SET actual_amt=? WHERE ref_no=? "
                args = [amount, ref_no.zfill(4)]
                try:    
                    self.cur.execute(sql,args)
                    self.conn.commit()
                except Exception. msg:
                    tkMessageBox.showerror("Update Amount", "%s while updating record" %msg)
                else:
                    tkMessageBox.showinfo("Update Amount", "Record updated successfully")
                    self.get_payment_data(defaulter=defaulter,save_msg=True)
            else:
                sql = "Select id from member_data where ref_no=?"
                args=[str(ref_no.zfill(4))]
                self.cur.execute(sql,args)
                rec = self.cur.fetchone() 
                if rec:
                    id = rec[0]
                    sql = """
                         ...
                         """
                    parameters = [id,string.capwords(actual_month),actual_year,amount,expected_amt,name,ref_no.zfill(4)]
                    try:
                        self.cur.execute(sql, parameters)
                        self.conn.commit()
                    except Exception, msg:
                    ...
                    tkMessageBox.showerror("Update Amount", "Could not update record")
    
    def display_finance_data(self,ix,text):
        ...
        if text:
            ...
            name = text[5]
            ref_no = text[6]
            label_0_list=tk.Label(self.checkbox_pane.interior, text="%s" %id, bg=display_bg, width=5)
            label_1_list=tk.Label(self.checkbox_pane.interior, text="%s" %str(name), bg=display_bg, wraplength=150)
            label_2_list=tk.Label(self.checkbox_pane.interior, text="%s" %str(expected_amt), bg=display_bg)
            label_3_list=tk.Label(self.checkbox_pane.interior, text="%s" %str(donated_amt), bg=display_bg)
            label_4_list=tk.Label(self.checkbox_pane.interior, text="%s" %str(donated_amt-expected_amt), bg=display_bg)
            label_5_list=tk.Label(self.checkbox_pane.interior, text="%s" %str(ref_no), bg=display_bg)
            label_1_list.grid(row=ix+2, column=0)
            label_2_list.grid(row=ix+2, column=2)
            label_3_list.grid(row=ix+2, column=3)
            label_4_list.grid(row=ix+2, column=4)
            label_5_list.grid(row=ix+2, column=1)
    
    def display_payments_data(self,ix,text):
        label0 = tk.Label(self.checkbox_pane.interior, text=self.selected_month.get(), width=30,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
        label1 = tk.Label(self.checkbox_pane.interior, text=self.selected_year.get(), width=12,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
        label2 = tk.Label(self.checkbox_pane.interior, text="Name", width=30,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
        label3 = tk.Label(self.checkbox_pane.interior, text="Ref. No", width=12,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
        label4 = tk.Label(self.checkbox_pane.interior, text="Expected Amt.", width=12,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
        label5 = tk.Label(self.checkbox_pane.interior, text="Actual Amt.", width=12,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
    ...
    def prepare_letter(self, all_comm_recs, from_mem_page=False, save_msg=False):
        root.title("Verein Manager/Prepare Letter")
        if self.selected_lang.get() == "":
            ...
            if from_mem_page:
                ...
                back_but.config(command=lambda: self.check_current_payments(year=self.selected_year.get()))
    ...
    def prepare_for_save(self, all_rec, file_format, save_msg=False):
        ...
        if text == "":
            ...
            if temp_avail:
                ...
                tkMessageBox.showerror("Template Not Found", "Template is missing at path %s" %os.path.abspath(os.path.join(os.getcwd(),'template','masjid_template.%s'%file_format)))
    ...
    def select_language(self, all_comm_recs,from_mem_page=False, save_msg=False):
        root.title("Verein Manager/Select Language")
    ...
    def friday_data(self, kill_checkpane=""):
        root.title("Verein Manager/Finance/Friday Data")
        if kill_checkpane:
            self.checkbox_pane.destroy()
        self.first_frame.destroy()
        self.first_frame = tk.Frame(self.master, bg=background_color)
        ...
        but_frame = tk.Frame(self.first_frame, bg=background_color)
        but_frame.grid(row=0, column=0, padx=250, pady=150)
        update_but = tk.Button(but_frame, text = 'Update', command = self.update_friday_data, height = 1, width = 12)
        update_but.grid(row=2, column=0, padx=25, pady=5)
        hist_but = tk.Button(but_frame, text = 'View History', command = self.view_friday_history, height = 1, width = 12)
        hist_but.grid(row=2, column=1, padx=5, pady=5)
        back_but = tk.Button(but_frame, text = 'Back', command = self.manage_finance, height = 1, width = 12)
        back_but.grid(row=3, column=0, padx=5, pady=15)
    
    def view_friday_history(self,kill_checkpane=""):
        root.title("Verein Manager/Finance/Friday Data/History")
        self.first_frame.destroy()
        self.first_frame = tk.Frame(self.master, bg=background_color)
        self.first_frame.grid(row=5, column=0)
        lab_fr = tk.Frame(self.first_frame, bg=background_color)
        ...
        but_fr = tk.Frame(self.first_frame, bg=background_color)
        but_fr.grid(row=1, column=3, padx=100,pady=1)
        save_as_rpt = tk.Button(but_fr, text="Save as PDF", command=lambda: self.generate_report(fri_data=all_data), width=15, height=1)
        save_as_rpt.grid(row=1, column=0, padx=25, pady=25)
        back_but = tk.Button(but_fr, text="Back", command= lambda: self.friday_data(kill_checkpane=True), width=15, height=1)
        back_but.grid(row=1, column=1)
        lab_fr = tk.Frame(self.first_frame, bg=background_color)
        lab_fr.grid(row=0, column=3, padx=100,pady=1)
        self.nfd_label=tk.Label(lab_fr,bg=background_color)
        self.nfd_label.grid(row=0,column=0,columnspan=2, sticky=tk.N,padx=5)
        self.cur.execute("Select * from friday_data")
        all_data = self.cur.fetchall()
        self.cur.execute("Select sum(amount) from friday_data")
        total_amount = self.cur.fetchall()
        if total_amount[0][0]:
            friday_total_amount = total_amount[0][0]
        else:
            friday_total_amount = 0
        display_frame = tk.Frame(self.first_frame, bg=background_color)
        display_frame.grid(row=1, column=4, padx=25,pady=1,rowspan=2)
        total_exp_label = tk.Label(display_frame, text="Total Amount :", width=15, justify=tk.LEFT,anchor=tk.W, bg=background_color)
        total_exp_val_label = tk.Label(display_frame, text=friday_total_amount, width=15, justify=tk.LEFT,anchor=tk.W, bg=background_color)
        total_exp_label.grid(row=1, column=0, sticky=tk.W, padx=15, pady=5)
        total_exp_val_label.grid(row=1, column=1, sticky=tk.W, padx=15, pady=5)
        self.checkbox_pane = ScrollableFrame(self.master, bg=display_bg, relief=tk.RIDGE)
        self.checkbox_pane.grid(row=0, column=0, rowspan=5, columnspan=5, padx=5, pady=5)
        ...
        if not all_data:
            self.nfd_label.config(text="Database has no records to display!")
            save_as_rpt.config(state=tk.DISABLED)
            self.prev_but.config(state=tk.DISABLED)
            self.next_but.config(state=tk.DISABLED)
            self.display_friday_data("","")
        else:
            self.nfd_label.config(text="")
            save_as_rpt.config(state=tk.NORMAL)
            if self.max_len <= self.max_num:
                self.max_num = self.max_len
                self.next_but.config(state=tk.DISABLED)
                self.prev_but.config(state=tk.DISABLED)
            for i in range(self.start_num,self.max_num,1):
                self.display_friday_data(i,all_data[i])
            self.recs_label.config( text="Displaying Records : %d to %d of %d" %(self.start_num+1,i+1,self.max_len))
            if len(all_data) > self.data_length:
                self.next_but.config(state=tk.NORMAL)
                self.prev_but.config(state=tk.DISABLED)
                self.next_but.config(command=lambda: self.show_next(all_data,friday_data=True))
                self.prev_but.config(command =lambda: self.show_previous(all_data,friday_data=True))
        
    def display_friday_data(self,ix,text):    
        label0 = tk.Label(self.checkbox_pane.interior, text="No.", width=5,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
        label1 = tk.Label(self.checkbox_pane.interior, text="Date", width=15,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
        label2 = tk.Label(self.checkbox_pane.interior, text="Amount", width=10,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
        label0.grid(row=0, column=0, sticky=tk.E, pady=15)
        label1.grid(row=0, column=1, sticky=tk.E, pady=15)
        label2.grid(row=0, column=2, sticky=tk.E)
        
        if text:
            label1 = tk.Label(self.checkbox_pane.interior, text=text[0], width=5,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
            label2 = tk.Label(self.checkbox_pane.interior, text=text[1], width=15,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
            label3 = tk.Label(self.checkbox_pane.interior, text=text[2], width=10,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
            label1.grid(row=ix+2, column=0, sticky=tk.E)
            label2.grid(row=ix+2, column=1, sticky=tk.E)
            label3.grid(row=ix+2, column=2, sticky=tk.E)
        
    def update_friday_data(self, kill_checkpane=""):   
        root.title("Verein Manager/Finance/Friday Data/Update") 
        self.first_frame.destroy()
        self.first_frame = tk.Frame(self.master, bg=background_color)
        self.first_frame.grid(row=5, column=0)
        field_frame = tk.Frame(self.first_frame, bg=background_color)
        field_frame.grid(row=5, column=0, padx=250, pady=150)
        but_frame = tk.Frame(self.first_frame, bg=background_color)
        but_frame.grid(row=6, column=0)
        date_label = tk.Label(field_frame,bg=background_color,text="Date (DD/MM/YYYY): ")
        date_label.grid(row=4,column=0,sticky=tk.W)
        friday_date = tk.Entry(field_frame,width=17)
        friday_date.grid(row=4,column=1,sticky=tk.W,padx=2)
        pass_label = tk.Label(field_frame,bg=background_color,text="Amount: ")
        pass_label.grid(row=5,column=0,sticky=tk.W, padx=10, pady=10)
        friday_amt = tk.Entry(field_frame,width=5)
        friday_amt.grid(row=5,column=1,sticky=tk.W,padx=1)
        hist_but = tk.Button(but_frame, text = 'Add Data', command = lambda: self.add_friday_data(friday_date.get(), friday_amt.get()), height = 1, width = 12)
        hist_but.grid(row=5, column=0, padx=10)
        back_but = tk.Button(but_frame, text = 'Back', command = self.friday_data, height = 1, width = 12)
        back_but.grid(row=5, column=1)
    
    def add_friday_data(self, f_date, f_amt):
        date,error_msg = self.validate_date_fields(f_date)
        if not date:
            tkMessageBox.showerror("Invalid Date", error_msg)
            date_valid = False
        else:
            date_valid = True
            try:
                new_f_amt = float(f_amt)
                amt_valid = True
            except Exception, msg:
                amt_valid = False
                tkMessageBox.showerror("Invalid Amount", "Enter valid amount!")
        if date_valid and amt_valid:
            self.cur.execute("SELECT * FROM friday_data where Id = (SELECT MAX(Id) from friday_data)")
            rec = self.cur.fetchone() 
            if rec:
                max_id = rec[0]
                id = max_id + 1
                ...
                id =1
            sql = "INSERT into friday_data VALUES (?,?,?)"
            parameters = [id, f_date, f_amt]
            try:
                self.cur.execute(sql, parameters)
                self.conn.commit()
            except Exception, msg:
                tkMessageBox.showerror("Error", "%s occurred while adding Friday data! " %str(msg))
            else:
                tkMessageBox.showinfo("Add Data", "Record added successfully")
    
    
    def select_month_year(self, method_to_call, kill_checkbox=False):
        if kill_checkbox:
            self.checkbox_pane.destroy()
        root.title("Verein Manager/Finance/Select Month-Year")
        self.first_frame.destroy()
        self.first_frame = tk.Frame(self.master, bg=background_color)
        self.first_frame.grid(row=5, column=0)
        dis_frame = tk.Frame(self.first_frame, bg=background_color)
        dis_frame.grid(row=1,column=1, padx=250, pady=150,columnspan=5)
        ...
        selected_year = tk.Entry(dis_frame, width=10)
        selected_year.grid(row=5,column=5,padx=25)
        selected_year.insert(0, datetime.date.today().strftime("%Y"))
        but_frame = tk.Frame(self.first_frame, bg=background_color)
        but_frame.grid(row=2,column=1, padx=5, pady=10,columnspan=5)
        if method_to_call == "finance":
            gen_rpt = tk.Button(but_frame, text="View and Update", command=lambda: self.update_finance_data(year=selected_year.get()), width=15, height=1)
            gen_rpt.grid(row=6,column=2,pady=5,padx=5)
        elif method_to_call == "check_payments":
            gen_rpt = tk.Button(but_frame, text="View", command=lambda: self.check_current_payments(year=selected_year.get()), width=15, height=1)
            gen_rpt.grid(row=6,column=2,pady=5,padx=5)
        bck_but = tk.Button(but_frame, text="Back", command=self.manage_finance, width=15, height=1)
        bck_but.grid(row=6,column=3,pady=5,sticky=tk.NW)
    
        
    def update_finance_data(self, year, kill_checkpane=""):
        if int(year) not in valid_years:
            tkMessageBox.showerror("View Monthly Data", "Enter a valid year to view data")
        else:
            self.selected_year.set(year)
        
            root.title("Verein Manager/Finance/Update")
            if kill_checkpane:
                self.checkbox_pane.destroy()
            self.first_frame.destroy()
            self.first_frame = tk.Frame(self.master, bg=background_color)
            ...
            sql="Select * from member_data where id not in (select id from finance where month=? and year=?)"
            #parameters = [datetime.date.today().strftime("%B"), datetime.date.today().strftime("%Y")]
            parameters = [self.selected_month.get(), year]
            self.cur.execute(sql,parameters)
            all_data = self.cur.fetchall()
            self.checkbox_pane = ScrollableFrame(self.master, bg=display_bg, relief=tk.RIDGE)
            self.checkbox_pane.grid(row=0, column=0, rowspan=5, columnspan=5, padx=5, pady=5)
    ...
    def show_previous(self,all_data,display=False,payments=False,comm_history=False,friday_data=False,historical_payments=False,view_finance=False):
        self.checkbox_pane.destroy()
        self.checkbox_pane = ScrollableFrame(self.master, bg=display_bg, relief=tk.RIDGE)
        self.checkbox_pane.grid(row=0, column=0, rowspan=5, columnspan=5, padx=5, pady=5)
        if display:
            ...
            func = "self.display_friday_data(x,all_data[i])"
        elif historical_payments:
            func = "self.display_historical_finance_data(x,all_data[i])"
        elif view_finance:
            func = "self.display_finance_data(x,all_data[i])"
        else:
    ...
    def show_next(self,all_data,display=False,payments=False,comm_history=False,friday_data=False,historical_payments=False,view_finance=False):
        self.checkbox_pane.destroy()
        self.checkbox_pane = ScrollableFrame(self.master, bg=display_bg, relief=tk.RIDGE)
        self.checkbox_pane.grid(row=0, column=0, rowspan=5, columnspan=5, padx=5, pady=5)
        if display:
    ...
    def show_data(self,ix,text):  
        ...
        if text:
            ...
            self.month.append(tk.StringVar(self.checkbox_pane.interior, value=self.selected_month.get()))
            self.year.append(tk.StringVar(self.checkbox_pane.interior, value=self.selected_year.get()))
    ...
    def view_all_history(self, kill_check=""):
        root.title("Verein Manager/History")
        if kill_check:
            self.checkbox_pane.destroy()
        self.first_frame.destroy()
        self.first_frame = tk.Frame(self.master, bg=background_color)
    ...
    def view_deleted_members(self):
        root.title("Verein Manager/History/Deleted Members")
        global all_member_data
        all_member_data=[]
        self.first_frame.destroy()
        self.first_frame = tk.Frame(self.master, bg=background_color)
    ...
    def view_yearly_financial_data(self, kill_checkbox=False):
        root.title("Verein Manager/Finance/View Yearly Data")
        if kill_checkbox:
            self.checkbox_pane.destroy()
        self.first_frame.destroy()
        self.first_frame = tk.Frame(self.master, bg=background_color)
        self.first_frame.grid(row=5, column=0)
        dis_frame = tk.Frame(self.first_frame, bg=background_color)
        dis_frame.grid(row=1,column=1, padx=250, pady=150)
        l1= tk.Label(dis_frame, text="Enter year :", bg=background_color)
        l1.grid(row=0,column=0, padx=25,sticky=tk.W)
        year_field = tk.Entry(dis_frame, width=8)
        year_field.grid(row=0,column=1,sticky=tk.W,padx=5)
        year_field.insert(0, datetime.date.today().strftime("%Y"))
        but_frame = tk.Frame(self.first_frame, bg=background_color)
        but_frame.grid(row=2,column=1)
        gen_rpt = tk.Button(but_frame, text="View Data", command=lambda: self.show_yearly_finance_data(year_field.get()), width=12, height=1)
        gen_rpt.grid(row=0,column=0,pady=5,padx=15)
        bck_but = tk.Button(but_frame, text="Back", command=self.manage_finance, width=12, height=1)
        bck_but.grid(row=0,column=1,pady=5,sticky=tk.NW)
    
    def show_yearly_finance_data(self, year):
        if int(year) not in valid_years:
            tkMessageBox.showerror("View Yearly Data", "Enter a valid year in the range[2016-3000] to view data!")
        else:
            dict=OrderedDict()
            self.first_frame.destroy()
            self.first_frame = tk.Frame(self.master, bg=background_color)
            ...
            self.checkbox_pane.grid(row=0, column=0, rowspan=5, columnspan=5, padx=5, pady=5)
            dis_frame = tk.Frame(self.first_frame, bg=background_color)
            dis_frame.grid(row=1,column=1, padx=10, pady=15)
            months = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"]
            total_label = tk.Label(self.checkbox_pane.interior, text="Month ", width=15, justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
            total_exp_label = tk.Label(self.checkbox_pane.interior, text="Expected Amount ",  width=15,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
            total_act_label = tk.Label(self.checkbox_pane.interior, text="Actual Amount ",  width=15,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
            total_diff_label = tk.Label(self.checkbox_pane.interior, text="Difference ",  width=10,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
            total_label.grid(row=0,column=0,sticky=tk.W, padx=15, pady=7)
            total_exp_label.grid(row=0,column=1,sticky=tk.W, padx=15)
            total_act_label.grid(row=0,column=2,sticky=tk.W, padx=15)
            total_diff_label.grid(row=0,column=3,sticky=tk.W, padx=15)
            total_expected_year=0
            total_actual_year=0
            total_difference_year=0
            
            sql ="Select sum(donation) from member_data"
            parameters = []
            self.cur.execute(sql,parameters)
            total_exp_data = self.cur.fetchall()
            
            if total_exp_data[0][0] :
                total_expected_each_month = total_exp_data[0][0]
            else:
                total_expected_each_month=0
            
            for ix, each_month in enumerate(months):
                sql = "select sum(actual_amt) from finance where month=? and year=?"
                args=[each_month, year]
                self.cur.execute(sql,args)
                total_data = self.cur.fetchall()
                if total_data[0][0]:
                    expected_amt = total_expected_each_month
                    actual_amt = total_data[0][0]
                    difference_amt = actual_amt-expected_amt
                    total_expected_year += expected_amt
                    total_actual_year += actual_amt
                    total_difference_year += difference_amt
                    dict[each_month] = [expected_amt, actual_amt, difference_amt]
                else:
                     ...
                     total_actual_year += actual_amt
                     total_difference_year += difference_amt
                     dict[each_month] = [total_expected_each_month, actual_amt, difference_amt]
                month_label = tk.Label(self.checkbox_pane.interior, text=each_month, width=15, bg=display_bg)
                exp_label = tk.Label(self.checkbox_pane.interior, text=expected_amt, width=15, bg=display_bg)
                act_label = tk.Label(self.checkbox_pane.interior, text=actual_amt, width=15, bg=display_bg)
                diff_label = tk.Label(self.checkbox_pane.interior, text=difference_amt, width=10, bg=display_bg)
                month_label.grid(row=ix+1,column=0)
                exp_label.grid(row=ix+1,column=1)
                act_label.grid(row=ix+1,column=2)
                diff_label.grid(row=ix+1,column=3)
            self.report_for_year = year
            prepare_but = tk.Button(dis_frame, text = 'Save as PDF', command =lambda: self.generate_report(yearly_rpt=dict), height = 1, width = 12)
            prepare_but.grid(row=0, column=0, padx=5, pady=5)
            back_but = tk.Button(dis_frame, text="Back", command=lambda: self.view_yearly_financial_data(kill_checkbox=True), height=1, width=12)
            back_but.grid(row=0,column=1)
            display_frame = tk.Frame(self.first_frame, bg=background_color)
            display_frame.grid(row=1, column=3, padx=125,pady=1,rowspan=2)
            total_label = tk.Label(display_frame, text="Total ", width=15, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            total_exp_label = tk.Label(display_frame, text="Expected Amount ", width=15, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            total_act_label = tk.Label(display_frame, text="Actual Amount ", width=15, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            total_diff_label = tk.Label(display_frame, text="Difference ", width=10, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            total_exp_val_label = tk.Label(display_frame, text=total_expected_year, width=15, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            total_act_val_label = tk.Label(display_frame, text=total_actual_year, width=15, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            total_diff_val_label = tk.Label(display_frame, text=total_difference_year, width=10, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            total_exp_label.grid(row=1, column=0, sticky=tk.W, padx=15, pady=5)
            total_act_label.grid(row=2, column=0, sticky=tk.W, padx=15, pady=5)
            total_diff_label.grid(row=3, column=0, sticky=tk.W, padx=15, pady=5)
            total_label.grid(row=0, column=1, sticky=tk.W, padx=15, pady=5)
            total_exp_val_label.grid(row=1, column=1, sticky=tk.W, padx=15, pady=5)
            total_act_val_label.grid(row=2, column=1, sticky=tk.W, padx=15, pady=5)
            total_diff_val_label.grid(row=3, column=1, sticky=tk.W, padx=15, pady=5)
    
    def view_finance_data_by_month_year(self):
        root.title("Verein Manager/Finance/View Monthly Data")
        self.first_frame.destroy()
        self.first_frame = tk.Frame(self.master, bg=background_color)
        self.first_frame.grid(row=5, column=0)
        dis_frame = tk.Frame(self.first_frame, bg=background_color)
        ...
        but_frame = tk.Frame(self.first_frame, bg=background_color)
        but_frame.grid(row=2,column=1, padx=5, pady=10,columnspan=5)
        gen_rpt = tk.Button(but_frame, text="View Data", command=lambda: self.show_finance_data_month_year(selected_year.get()), width=15, height=1)
        gen_rpt.grid(row=6,column=2,pady=5,padx=5)
        bck_but = tk.Button(but_frame, text="Back", command=self.manage_finance, width=15, height=1)
        bck_but.grid(row=6,column=3,pady=5,sticky=tk.NW)
    
    def show_finance_data_month_year(self, year):
        if int(year) not in valid_years:
            tkMessageBox.showerror("View Monthly Data", "Enter a valid year to view data")
        else:
            self.selected_year.set(year)
            ...
            parameters = [self.selected_month.get(), self.selected_year.get()]
            self.cur.execute(sql, parameters)
            less_data_frm_fin = self.cur.fetchall()
            
            sql = "Select * from finance inner join member_data on finance.Id = member_data.Id where finance.actual_amt == finance.donated_amt and finance.month=? and finance.year =?"
            parameters = [self.selected_month.get(), self.selected_year.get()]
            self.cur.execute(sql, parameters)
            equal_data_frm_fin = self.cur.fetchall()
            ...
            all_data=data_frm_fin+not_paid_mem+less_data_frm_fin+equal_data_frm_fin

            sql ="Select sum(actual_amt) from finance where month=? and year=?"
            parameters = [self.selected_month.get(),self.selected_year.get()]
            self.cur.execute(sql,parameters)
            total_act_data = self.cur.fetchall()
            ...
            if total_act_data[0][0]:
                actual_total = total_act_data[0][0]
            else:
                actual_total = 0
            if total_exp_data[0][0]:
                expected_total = total_exp_data[0][0]
            else:
                 expected_total = 0
                
            difference_amt = actual_total - expected_total   
            ...
            self.recs_label=tk.Label(lab_fr,bg=background_color)
            self.recs_label.grid(row=0,column=0,columnspan=5, sticky=tk.W,padx=5,pady=5)
            button_fr = tk.Frame(self.first_frame, bg=background_color)
            button_fr.grid(row=1, column=3, padx=100,pady=1)
            display_frame = tk.Frame(self.first_frame, bg=background_color)
            display_frame.grid(row=1, column=4, padx=25,pady=1,rowspan=2)
            prepare_but = tk.Button(button_fr, text = 'Save as PDF', command = self.generate_report, height = 1, width = 12)
            prepare_but.grid(row=2, column=2, padx=5, pady=5)
            total_label = tk.Label(display_frame, text="Total ", width=15, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            total_exp_label = tk.Label(display_frame, text="Expected Amount ", width=15, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            total_act_label = tk.Label(display_frame, text="Actual Amount ", width=15, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            total_diff_label = tk.Label(display_frame, text="Difference ", width=10, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            total_exp_val_label = tk.Label(display_frame, text=expected_total, width=15, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            total_act_val_label = tk.Label(display_frame, text=actual_total, width=15, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            total_diff_val_label = tk.Label(display_frame, text=difference_amt, width=10, justify=tk.LEFT,anchor=tk.W, bg=background_color)
            total_exp_label.grid(row=1, column=0, sticky=tk.W, padx=15, pady=5)
            total_act_label.grid(row=2, column=0, sticky=tk.W, padx=15, pady=5)
            total_diff_label.grid(row=3, column=0, sticky=tk.W, padx=15, pady=5)
            total_label.grid(row=0, column=1, sticky=tk.W, padx=15, pady=5)
            ...
            total_act_val_label.grid(row=2, column=1, sticky=tk.W, padx=15, pady=5)
            total_diff_val_label.grid(row=3, column=1, sticky=tk.W, padx=15, pady=5)
            back_but = tk.Button(button_fr, text = 'Back', command = lambda: self.manage_finance(kill_checkbox=True), height = 1, width = 12)
            back_but.grid(row=2, column=3, padx=5, pady=5)
            lab_fr = tk.Frame(self.first_frame, bg=background_color)
            lab_fr.grid(row=0, column=3, padx=100,pady=1)
            self.nfd_label=tk.Label(lab_fr,bg=background_color)
            self.nfd_label.grid(row=0,column=1,columnspan=5, sticky=tk.W,padx=5)
            ...
            if not all_data:
                ...
                if self.max_len <= self.max_num:
                    ...
                    self.prev_but.config(state=tk.DISABLED)
                for i in range(self.start_num,self.max_num,1):
                    self.display_finance_data(i,all_data[i])
                self.recs_label.config( text="Displaying Records : %d to %d of %d" %(self.start_num+1,i+1,self.max_len))
                if len(all_data) > self.data_length:
                    self.next_but.config(state=tk.NORMAL)
                    self.next_but.config(command =lambda: self.show_next(all_data,view_finance=True))
                    self.prev_but.config(command =lambda: self.show_previous(all_data,view_finance=True))
                    self.prev_but.config(state=tk.DISABLED)
    ...
    def pdf_first_page(self, canvas, doc):
        ...
        if self.create_for_member:
            ...
            canvas.drawString(360,720,'Report generated on : %s' %str("{:%d.%m.%Y-%H:%M:%S}".format(datetime.datetime.now())))
            canvas.setLineWidth(.2)
            canvas.line(50,695,540,695)
        elif self.friday_report:
            canvas.drawString(65,720,"Finance report for: Friday Collections")
            canvas.drawString(360,720,'Report generated on : %s' %str("{:%d.%m.%Y-%H:%M:%S}".format(datetime.datetime.now())))
            canvas.setLineWidth(.2)
            canvas.line(50,695,540,695)
        elif self.yearly_report:
            canvas.drawString(65,720,"Finance report for Year : %s" %self.report_for_year)
            canvas.drawString(360,720,'Report generated on : %s' %str("{:%d.%m.%Y-%H:%M:%S}".format(datetime.datetime.now())))
            canvas.setLineWidth(.2)
            canvas.line(50,695,540,695)
        else:
            canvas.drawString(65,720,"Finance report for : %s %s" %(self.selected_month.get(), self.selected_year.get()))
            canvas.drawString(65,705,'Report generated on : %s' %str("{:%d.%m.%Y-%H:%M:%S}".format(datetime.datetime.now())))
            canvas.setLineWidth(.2)
            canvas.line(50,695,540,695)
    ...
    def create_report(self,inp_data):
        ...
        with open(os.path.abspath(os.path.join(dir_path,'temp.pdf')), "rb") as f:
            ...
            if self.create_for_member:
                ...
                outputStream = file(os.path.abspath(os.path.join(dir_path,"Finance_Report_Friday_%s.pdf" %("{:%H.%M.%S}".format(datetime.datetime.now())) )), "wb")
                tkMessageBox.showinfo("Report Created", "Report created at path: %s" %(os.path.abspath(os.path.join(dir_path,"Finance_Report_Friday_%s.pdf"
                                                                                                 %("{:%H.%M.%S}".format(datetime.datetime.now())) ))))
            elif self.yearly_report:
                outputStream = file(os.path.abspath(os.path.join(dir_path,"Finance_Report_for_year_%s_%s.pdf" %(self.report_for_year, "{:%H.%M.%S}".format(datetime.datetime.now())) )), "wb")
                tkMessageBox.showinfo("Report Created", "Report created at path: %s" %(os.path.abspath(os.path.join(dir_path,"Finance_Report_for_year_%s_%s.pdf"
                                                                                                 %(self.report_for_year, "{:%H.%M.%S}".format(datetime.datetime.now())) ))))
            else:
    ...
    def createLineItems(self,inp_data):
        if self.create_for_member:
            ...
            text_data = ["Sr. No.", "Date", "Amount"]
        elif self.yearly_report:
            text_data = ["Sr. No.", "Month", "Expected Amt.", "Paid Amt.", "Difference"]
            ek_list=[]
            for each_key in inp_data.keys():
                val_list = inp_data[each_key]
                val_list.append(each_key)
                ek_list.append(val_list)
            inp_data = ek_list
        else:
        ...
        for each_rec in inp_data:
            if self.create_for_member:
                ...
                line_data = [str(line_num),each_rec[1],each_rec[2]]
                total_difference = total_difference + float(each_rec[2])
            elif self.yearly_report:
                line_data = [str(line_num),each_rec[3],each_rec[0], each_rec[1], each_rec[2] ]
                total_difference = total_difference + float(each_rec[2])
                total_expected = total_expected + float(each_rec[0])
                total_paid = total_paid + float(each_rec[1])
        ...
        if self.friday_report:
            lst_val = ["","Total",total_difference]
            filler = ["","-----------------------------------------","-----------------------------------------"]
            col_width = [75, 175, 175, 0, 0, 0]
        elif self.yearly_report:
            lst_val = ["","Total",total_expected,total_paid,total_difference]
            filler = ["","-----------------------","-----------------------","-----------------------", "-----------------------"]
            col_width = [100, 100, 100, 100, 100, 100]
        else:
            lst_val = ["","","Total",total_expected,total_paid,total_difference]
            filler = ["","","----------------------------------------------------","------------------","------------------", "-----------------"]
            col_width = [50, 50, 190, 70, 70, 70]
        
        for each_item in lst_val:    
    ...
    def generate_report(self, mem_data="", mem_name="", ref_no="",fri_data="",yearly_rpt=""):
        self.create_for_member=False
        self.friday_report=False
        self.yearly_report=False
        self.member_name=""
        self.member_ref_no=""
        if mem_data:
            all_data = mem_data
            ...
            all_data = fri_data
            self.friday_report=True
        elif yearly_rpt:
            all_data = yearly_rpt
            self.yearly_report=True
        else:
            sql ="Select * from finance where month=? and year=?"
            parameters = [self.selected_month.get(),self.selected_year.get()]
            self.cur.execute(sql,parameters)
            paid_data = self.cur.fetchall()
            ...
            all_data = paid_data+not_paid_mem
    ...
    def finance_history(self, kill_checkbox=""):
        root.title("Verein Manager/History/Payments")
        if kill_checkbox:
            self.checkbox_pane.destroy()
        self.first_frame.destroy()
        self.first_frame = tk.Frame(self.master, bg=background_color)
    ...
    def view_history(self):
        if self.ref_no_by_user.get() == "":
            ...
            but_fr = tk.Frame(self.first_frame, bg=background_color)
            but_fr.grid(row=2, column=3, padx=100,pady=1,rowspan=2)
            save_as_rpt = tk.Button(but_fr, text="Save as PDF", command=lambda: self.generate_report(mem_data=all_data,mem_name=name,ref_no=ref_no), width=15, height=1)
            save_as_rpt.grid(row=1, column=0, padx=25, pady=25)
            back_but = tk.Button(but_fr, text="Back", command= lambda: self.finance_history(kill_checkbox=True), width=15, height=1)
            back_but.grid(row=1, column=1)
            lab_fr = tk.Frame(self.first_frame, bg=background_color)
            lab_fr.grid(row=0, column=3, padx=100,pady=1)
            self.nfd_label=tk.Label(lab_fr,bg=background_color)
            ...
            if not all_data:
                ...
                self.prev_but.config(state=tk.DISABLED)
                self.next_but.config(state=tk.DISABLED)
                self.display_historical_finance_data("","")
            else:
                self.nfd_label.config(text="")
                save_as_rpt.config(state=tk.NORMAL)
                ref_no = all_data[0][6]
                ...
                if self.max_len <= self.max_num:
                    ...
                    self.prev_but.config(state=tk.DISABLED)
                for i in range(self.start_num,self.max_num,1):
                    self.display_historical_finance_data(i,all_data[i])
                self.recs_label.config( text="Displaying Records : %d to %d of %d" %(self.start_num+1,i+1,self.max_len))
                if len(all_data) > self.data_length:
                    self.next_but.config(state=tk.NORMAL)
                    self.prev_but.config(state=tk.DISABLED)
                    self.next_but.config(command=lambda: self.show_next(all_data,historical_payments=True))
                    self.prev_but.config(command =lambda: self.show_previous(all_data,historical_payments=True))
                
    def display_historical_finance_data(self,ix,text):            
        label1 = tk.Label(self.checkbox_pane.interior, text="Name", width=15,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
        ...
        label6.grid(row=1, column=4, sticky=tk.E)
        if text:
            label1 = tk.Label(self.checkbox_pane.interior, text=text[5], width=20,wraplength=150,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
            label2 = tk.Label(self.checkbox_pane.interior, text=text[1], width=15,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
            label3 = tk.Label(self.checkbox_pane.interior, text=text[2], width=20,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
            label4 = tk.Label(self.checkbox_pane.interior, text=text[4], width=12,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
            label5 = tk.Label(self.checkbox_pane.interior, text=text[3], width=12,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
            label6 = tk.Label(self.checkbox_pane.interior, text=float(text[3]-text[4]), width=12,justify=tk.CENTER, borderwidth=1, relief=tk.SUNKEN)
            label1.grid(row=0, column=1, sticky=tk.E)
            label2.grid(row=ix+2, column=0, sticky=tk.E)
            label3.grid(row=ix+2, column=1, sticky=tk.E)
...
#             and donation_not_empty and date_of_birth_not_empty and date_joined_not_empty:
        if ref_no_not_empty and f_name_not_empty and donation_not_empty and date_of_birth_not_empty:
            ...
            if ans:
                ...
                sql = """ UPDATE member_data
                                    SET name=?, street=?, house_no=?, post_code=?, city=?, phone=?, date_of_birth=?, email=?, donation=? , 
                                     date_joined=?, comment=?
                                    WHERE Id = ? """
                parameters = [self.f_name.get(), self.street.get(), self.house_no.get(), self.post_code.get(), self.city.get(), 
                                    self.phone.get(), self.date_of_birth.get(), self.email.get(), donation, self.date_joined.get(),
